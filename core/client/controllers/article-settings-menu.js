...
var ArticleSettingsMenuController = Ember.ObjectController.extend({
    ...
    changeAuthor: function () {
        ...
        if (!selectedAuthor || selectedAuthor.get('id') === author.get('id')) {
            return;
        }
        model.set('author', selectedAuthor);

        //if this is a new article (never been saved before), don't try to save it
        if (this.get('isNew')) {
            return;
        }
    ...
    // observe titleScratch, keeping the article's slug in sync
    ...
    slugPlaceholder: Ember.computed(function (key, value) {
        ...
        //If the article has a slug, that's its placeholder.
        if (slug) {
            return slug;
        }
...
export default ArticleSettingsMenuController;
