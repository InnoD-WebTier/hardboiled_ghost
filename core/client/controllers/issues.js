...
var IssuesController = Ember.ArrayController.extend(PaginationControllerMixin, {
    ...
    //
    // this function will keep the issues list sorted when loading individual/bulk
    // models from the server, even if records in between haven't been loaded.
    // this can happen when reloading the page on the Editor or IssuesIssue routes.
    //
    // a custom sort function is needed in order to sort the issues list the same way the server would:
    //     status: ASC
    //     published_at: DESC
    //     updated_at: DESC
