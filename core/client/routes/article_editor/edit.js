import base from 'ghost/mixins/article-editor-route-base';

var ArticleEditorEditRoute = Ember.Route.extend(SimpleAuth.AuthenticatedRouteMixin, base, {
  classNames: ['editor'],
  ...
  model: function (params) {
    ...
    if (!_.isNumber(articleId) || !_.isFinite(articleId) || articleId % 1 !== 0 || articleId <= 0) {
      return this.transitionTo('error404', 'article_editor/' + params.article_id);
    }

    article = this.store.findById('article', articleId);

    if (article) {
      return article;
    }

    //FIXME: transition to corresponding issue
    return self.transitionTo('issues.index');
  },

  serialize: function (model) {
    return {article_id: model.get('id')};
  },
...
export default ArticleEditorEditRoute;
